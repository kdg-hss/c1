...
#######AUTOSCRIPT JULAK VPN VERSI 0.1#######
clear
wget -q ${julak}s5/limit.sh && chmod +x limit.sh && ./limit.sh
clear
cd
wget -q -O /usr/local/sbin/quota "${julak}s5/quota"
chmod +x /usr/local/sbin/quota
cd /usr/local/sbin/
sed -i 's/\r//' quota
clear
cd
wget -q -O /usr/bin/limit-ip "${julak}s5/limit-ip"
chmod +x /usr/bin/*
cd /usr/bin
sed -i 's/\r//' limit-ip
clear
wget -q -O /usr/bin/limit-ip-ssh "${julak}s5/limit-ip-ssh"
chmod +x /usr/bin/*
cd /usr/bin
