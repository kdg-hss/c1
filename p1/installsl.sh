...
setup_dnstt() {
	...
	mkdir -p /etc/slowdns
	cd /etc/slowdns
	wget -O dnstt-server "${julak}s6/dnstt-server" >/dev/null 2>&1
	chmod +x dnstt-server >/dev/null 2>&1
	wget -O dnstt-client "${julak}s6/dnstt-client" >/dev/null 2>&1
	chmod +x dnstt-client >/dev/null 2>&1
	./dnstt-server -gen-key -privkey-file server.key -pubkey-file server.pub
	chmod +x *
	wget -O /etc/systemd/system/client.service "${julak}s6/client" >/dev/null 2>&1
	wget -O /etc/systemd/system/server.service "${julak}s6/server" >/dev/null 2>&1
